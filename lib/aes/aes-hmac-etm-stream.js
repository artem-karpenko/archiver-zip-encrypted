...
function AesHmacEtmStream(options = {key: null, salt: null, transformOptions: {}}) {
    ...
    this.cipher2 = new aesjs.ModeOfOperation.ctr(new Uint8Array(this.key.aesKey),
        new aesjs.Counter(new Uint8Array(Buffer.from('01000000000000000000000000000000', 'hex'))));
...
AesHmacEtmStream.prototype._transform = function (data, encoding, cb) {
    ...
    let encoded = this.cipher2.encrypt(new Uint8Array(data));

    if (encoded.length > 0) {
        this.hmac.update(encoded);
        const pushed = this.push(encoded);
        // console.log(pushed);
