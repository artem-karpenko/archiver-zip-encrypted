...
CryptoStream.prototype._transform = function (data, encoding, cb) {
    if (!this.headerSent) {
        ...
        header.write("01020304050607080910", "hex")
        header.writeUInt16LE(this.crc.readUInt16BE(0), 10);
        console.log(`[ZipCrypto] Plain header: ${header.toString('hex')} of ${header.length} bytes`);
